...
void extract_circuit_util(int s, int d, vector <int> v[], bool visited[], int path[], int path_index, bool extracted[])
{
    visited[s] = true;
    path[path_index] = s;
    path_index++;
    ...
    else
    {
        ...
        {
            if (!visited[v[s][i]] && !extracted[v[s][i]])
            {
                extract_circuit_util(v[s][i], d, v, visited, path, path_index, extracted);
            }
        }
    }
    visited[s] = false;
...
void extract_circuits(int s, int d, vector <int> v[], bool extracted[])
{
    bool visited[200] = {false};
    int path[300];
    int path_index = 0;

    extract_circuit_util(s, d, v, visited, path, path_index, extracted);
}
...
int main()
{
    ...
    cin>>vertex;
    bool extracted[300] = {false};

    for (int i=0; i<v[vertex].size(); i++)
    {
        extract_circuits(v[vertex][i], vertex, v, extracted);
        extracted[v[vertex][i]] = true;
    }
}